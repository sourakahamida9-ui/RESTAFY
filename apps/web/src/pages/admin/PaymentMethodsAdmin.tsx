...
export default function PaymentMethodsAdmin() {
  ...
  const handleSave = async () => {
    try {
      ...
      setMethods(methods.map(m => m.id === editing ? { ...m, ...formData } as PaymentMethod : m));
      setSuccess('Méthode de paiement mise à jour');
      setEditing(null);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Êtes-vous sûr de vouloir supprimer cette méthode ?')) return;
  ...
  if (loading) {
    return (
      <div className="r-page-shell !max-w-none bg-gray-50 p-8 flex items-center justify-center">
        <RestafyLoader fullscreen={false} message="Chargement des moyens de paiement…" size="md" />
      </div>
    );
  }
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600">Gérer les méthodes de paiement disponibles</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="px-6 py-4 text-left font-bold">Template USSD</th>
              ...
              <tbody>
                {methods.map(method => (
                  <tr key={method.id} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="px-6 py-4 font-mono text-xs">{method.code}</td>
                    <td className="px-6 py-4 font-bold">{method.name}</td>
                    <td className="px-6 py-4 font-mono text-xs text-gray-500">{method.ussd_template || '-'}</td>
                    ...
                    <td className="px-6 py-4">{method.sort_order}</td>
                    <td className="px-6 py-4">
        ...
        <AnimatePresence>
          {editing && (
            ...
            >
              ...
              >
                <h2 className="text-xl font-bold mb-4">Modifier la méthode</h2>
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold mb-1">Template USSD</label>
                    <input
                      type="text"
