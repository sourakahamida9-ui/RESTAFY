...
export default function SuperAdminUsers() {
    ...
    try {
      ...
      if (updatedUser?.email) {
        sendUserStatusEmail({
          ...
          else { if (import.meta.env.DEV) console.log('[Email] Notification envoyée à:', updatedUser.email); }
        });
      }
    } catch (error) {
      if (import.meta.env.DEV) console.error('Error updating user ban status:', error);
  ...
  const handleDeleteUser = async () => {
    ...
    if (userToDelete.role === 'super_admin') {
      toast.error('Impossible de supprimer un compte super admin');
      return;
    }
