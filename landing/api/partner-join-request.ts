/**
 * POST — formulaire « Rejoindre Restafy » sur la vitrine (projet Vercel `landing/`).
 * Aligner avec `api/partner-join-request.ts` à la racine du monorepo.
 * Variables Vercel sur le projet landing : SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY,
 * optionnel PARTNER_JOIN_CORS_ORIGINS.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (error) {
    console.error('[landing/partner-join-request]', error);
    return res.status(500).json({
      ok: false,
